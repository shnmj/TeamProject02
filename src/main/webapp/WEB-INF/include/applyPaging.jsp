  ...
  <div id="paging" style="margin: 20px 0px; border:0px solid black;">
    <table style="width:100%;height:25px;text-align:center"  >
     ...
     <c:if test="${ startnum gt 1 }">
       <a href="/Mypage/Apply?user_id=${ user_id }&nowpage=1">⏮</a>
       <a href="/Mypage/Apply?user_id=${ user_id }&nowpage=${ startnum - 1 }">
       ⏪
       </a>
     </c:if>
    ...
    <c:choose>
        <c:when test="${pagenum eq searchVo.pagination.currentPage}">
            <a href="/Mypage/Apply?user_id=${user_id}&nowpage=${pagenum}" style="font-weight: bold; color: blue; text-decoration: underline;">
                ${pagenum}
            </a>
        </c:when>
        <c:otherwise>
            <a href="/Mypage/Apply?user_id=${user_id}&nowpage=${pagenum}">
                ${pagenum}
            </a>
        </c:otherwise>
    </c:choose>
    &nbsp;&nbsp;
     ...
     <c:if test="${ totalpagecount != endnum }">
       <a href="/Mypage/Apply?user_id=${ user_id }&nowpage=${ endnum + 1 }">
       ⏩
       </a>
       <a href="/Mypage/Apply?user_id=${ user_id }&nowpage=${ totalpagecount }">⏭</a>
     </c:if>    
